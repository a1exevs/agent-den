...
- A component file contains only imports and the component class — no top-level `const`, `type`, `interface` or
  functions. Constants go to the slice's `config/`, types and logic to `model/` (with a test), helpers to `lib/`
  (ESLint `no-restricted-syntax` on `ui/**`).
